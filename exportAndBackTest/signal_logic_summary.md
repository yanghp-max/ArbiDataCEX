...
2. 与阈值比较（默认扫描 `0, 1, 2, 3, 4`）：
   - `-a+b`：`open_z >= z_open_ab`
   - `+a-b`：`open_z >= z_open_ba`
3. 都不满足 → 跳过
...
用锁定的 `direction + branch` 计算 open_z、close_z：

| 锁定方向 | 加仓条件 | 平仓条件 |
|----------|----------|----------|
| `-a+b` | `open_z >= z_open_ab` | `close_z >= z_open_ba` |
| `+a-b` | `open_z >= z_open_ba` | `close_z >= z_open_ab` |
...
开、平仓均为 **`>=` 阈值**（与脚本参数 `z_open_ab_list`、`z_open_ba_list` 一致）。
...
开仓额外限制（平仓不适用 max_position 截断逻辑中的「无仓可开」分支，但平仓 qty 不超过当前持仓）：
...
|------|------|
| `orders` | 总成交次数（开 + 平） |
| `orders_side_ab` / `orders_side_ba` | 各方向开仓次数 |
